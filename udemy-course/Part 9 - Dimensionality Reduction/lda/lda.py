...
# classifier is now fitted to a training dataset that has only two features
...
ld1_range = np.arange(x_set[:, 0].min()-1, x_set[:, 0].max()+1, 0.01)
ld2_range = np.arange(x_set[:, 1].min()-1, x_set[:, 1].max()+1, 0.01)

x1, x2 = np.meshgrid(ld1_range, ld2_range)
